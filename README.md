...
| [`clear-pr`](./clear-pr) | Write PR titles and descriptions in anandyandawang's house style: an imperative title (like a commit subject, no ticket prefix) and a single `## Summary` section — 25-100 words (fewer is better), at least two paragraphs: the current state of things ("Currently, …") then what this PR does ("This PR introduces …"). Simple, plain, natural English — no fluff; backticks/code sparingly, only for particularly important files. Auto-applies when opening a PR; invoke `/clear-pr` to apply the format on demand. |
...
| [`orchestrator`](./orchestrator) | The heaviest model plans, cheaper models code. The main-loop model — whatever tops the current lineup — is orchestrator only: plan, decompose, write rich delegation prompts. Every coding subagent gets an explicit cheaper `model` override, picked by role from the current lineup: one tier down is the workhorse for nearly all delegated work (coding included), the mid tier only for the simplest tasks (never orchestrating), the lowest tier essentially unused — and never spawn your own tier. Then the orchestrator tests and reviews itself, delegating fixes back down in a loop until green. Direct edits only when delegating costs more than the change. Always-on via hooks. |
| [`ultracodinator`](./ultracodinator) | Ultracode's standing opt-in, run with right-sized agents. Binds only while ultracode is on for the session; dormant otherwise. Every substantive task runs as a Workflow orchestration, but the heaviest model in the lineup never rides in the fleet — it eats tokens like nothing else and exists for coordination alone: every workflow `agent()` call and every subagent carries an explicit `model` override below that tier. No fixed tier taxonomy below the coordinator — it judges each stage and right-sizes model and effort itself: cheap and low for mechanical stages, stronger for implementation and hard verification; when in doubt, size up. The coordinator keeps planning, scripts, stage prompts, final review, and synthesis. Always-on via hooks. |
| [`no-comments`](./no-comments) | Write code with **zero comments, ever**. No `//`, no `/* */`, no doc comments or docstrings, no `#`/`--`/`<!-- -->`, no TODO/Note/FIXME, no commented-out code — in any language. Every would-be comment becomes better code: clearer names, smaller functions, explicit types, enums, well-named constants. Only machine-read directives (shebangs, linter/pragma directives) may use comment syntax. Always-on via hooks. Don't run alongside `clear-comments` — pick one. |
