...
| [`ultracodinator`](./ultracodinator) | Ultracode's standing opt-in, run with right-sized agents. Binds only while ultracode is on for the session; dormant otherwise. Every substantive task runs as a Workflow orchestration, but the heaviest model in the lineup never rides in the fleet — it eats tokens like nothing else and exists for coordination alone: every workflow `agent()` call and every subagent carries an explicit `model` override sized to its task — the mid tier is the workhorse and default for implementation and verification, the high tier (one below the coordinator) is reserved for the most complex stages, the low tier takes the simplest mechanical stages. The coordinator keeps planning, scripts, stage prompts, final review, and synthesis. Always-on via hooks. |
